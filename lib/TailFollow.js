...
export default class TailFollow extends Readable {
  constructor (filePath, opts={}) {
    ...
    this._tailPositionForBufferMap = new WeakMap()
  ...
  _readToEnd () {
    ...
    fs.read(this._fd, buf, 0, buf.length, this._tailPosition, (err, bytesRead) => {
      ...
      const slicedBuffer = buf.slice(0, bytesRead)
      this._tailPositionForBufferMap.set(slicedBuffer, this._tailPosition)
      this.push(slicedBuffer)
  ...
  positionForBuffer (buf) {
    return this._tailPositionForBufferMap.get(buf)
  }
}
